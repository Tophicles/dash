...
// Helper to fetch user data (Policy, Configuration) from MB/JF
function fetchUser($baseUrl, $apiKey, $userId) {
...
// 1.5 Handle Policy/Configuration Migration (MB/JF -> MB/JF only)
if ($sourceType !== 'plex') {
    sendProgress("Copying user policy and configuration from source to target...", "info");
    $sourceUserData = fetchUser($sourceBaseUrl, $sourceToken, $sourceUserId);

    if ($sourceUserData && isset($sourceUserData['Policy']) && isset($sourceUserData['Configuration'])) {
        $sourcePolicy = $sourceUserData['Policy'];
        $sourceConfiguration = $sourceUserData['Configuration'];

        // Update Target Policy
        $urlPolicy = "$targetBaseUrl/Users/$targetUserId/Policy";
        $chPolicy = curl_init($urlPolicy);
        curl_setopt($chPolicy, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($chPolicy, CURLOPT_POST, true);
        curl_setopt($chPolicy, CURLOPT_POSTFIELDS, json_encode($sourcePolicy));
        ...
        $urlConfig = "$targetBaseUrl/Users/$targetUserId/Configuration";
        $chConfig = curl_init($urlConfig);
        curl_setopt($chConfig, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($chConfig, CURLOPT_POST, true);
        curl_setopt($chConfig, CURLOPT_POSTFIELDS, json_encode($sourceConfiguration));
        curl_setopt($chConfig, CURLOPT_HTTPHEADER, [
            "X-Emby-Token: $targetApiKey",
            "X-MediaBrowser-Token: $targetApiKey",
            "Content-Type: application/json",
            "Accept: application/json"
        ]);
        curl_setopt($chConfig, CURLOPT_SSL_VERIFYPEER, false);
        curl_exec($chConfig);
        curl_close($chConfig);

        sendProgress("User policies (Live TV, downloading, etc.) synced.", "success");
    } else {
        sendProgress("Could not fetch user policies from source, skipping.", "error");
    }
}
